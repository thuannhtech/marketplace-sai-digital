...
- **sitecore-marketplace-sdk/client** - using graphqr
- **Sitecore Marketplace SDK** - Using `@sitecore-marketplace-sdk/client` and `@sitecore-marketplace-sdk/xmc` for marketplace and XM Cloud data operations.
- **Tailwind CSS** - Utility-first CSS with container queries (@container)
- **blok.sitecore** - Modern component library
...
- **Separation of Concerns:** Keep the Admin Portal logically and structurally separate from the public-facing application, possibly under a specific route group like `(admin)` or a dedicated sub-application.
- **Layout:** Implement a standard dashboard layout consisting of a persistent Sidebar/Navigation, a Header (with user profile/settings), and a Main Content Area.
- **Authentication & Authorization:** Enforce strict RBAC (Role-Based Access Control). Ensure all admin API routes and page components are protected and validate user roles before rendering or processing data.

### UI Components & Styling
- **Shadcn/ui & Tailwind CSS:** Continue using Tailwind for utility-first styling and integrate Shadcn/ui for complex admin components (Data Tables, Dialogs, Selects, Toasts, Form controls).
- **Data Tables:** Use a robust table implementation (e.g., `@tanstack/react-table`) supporting pagination, sorting, filtering, and row selection.
- **Forms and Validation:** Use `react-hook-form` in combination with `zod` schema validation for all data entry forms. Keep form state predictable and robust.
- **Data Visualization:** Use charting libraries (like `recharts` or `chart.js`) for dashboard analytics when needed.

### State Management & Data Fetching
- **Marketplace SDK:** Utilize `@sitecore-marketplace-sdk/client` (v0.2.0+) and `@sitecore-marketplace-sdk/xmc` exclusively for interacting with the marketplace API and XM Cloud data operations.
- **Server State:** Prefer React Server Components or robust data fetching strategies for caching, synchronizing, and updating server state. Keep client-side state minimal.
- **Optimistic Updates:** Implement optimistic UI updates for common admin actions (approve, delete, toggle status) to improve perceived performance.
- **Error Handling:** Display clear error states using Toasts or Error Boundaries without breaking the application layout.

### Performance & Security
- **Data Fetching:** Do not over-fetch data. Implement server-side pagination and filtering for large datasets.
- **Security:** Ensure any sensitive operations require proper permissions. Inputs MUST be sanitized and validated server-side.
